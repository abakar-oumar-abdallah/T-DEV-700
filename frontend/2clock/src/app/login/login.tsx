  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setError(translateError(result.error || 'Échec de la connexion'));
      }
    } catch (error) {
      console.error('Login error:', error);
      setError(translateError('Erreur réseau. Veuillez réessayer.'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="rounded-xl shadow-2xl p-8 space-y-6 bg-background">
          <div className="text-center">
            <h2 className="text-2xl font-semibold font-poppins text-foreground">Bienvenue</h2>
            <p className="text-sm mt-2 font-poppins text-secondary">Connectez-vous à votre compte</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="email" className="block text-sm font-medium font-poppins text-foreground">Email</label>
              <input id="email" type="email" placeholder="votre.email@exemple.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 border border-secondary rounded-lg font-poppins text-base bg-background text-foreground placeholder-secondary/60 focus:ring-2 focus:ring-primary/50 focus:border-primary focus:outline-none transition-all duration-200" />
            </div>
            <div className="space-y-2">
              <label htmlFor="password" className="block text-sm font-medium font-poppins text-foreground">Mot de passe</label>
              <div className="relative">
                <input id="password" type={showPassword ? "text" : "password"} placeholder="Entrez votre mot de passe" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full px-4 py-3 pr-12 border border-secondary rounded-lg font-poppins text-base bg-background text-foreground placeholder-secondary/60 focus:ring-2 focus:ring-primary/50 focus:border-primary focus:outline-none transition-all duration-200" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-secondary hover:text-foreground transition-colors" aria-label={showPassword ? "Masquer le mot de passe" : "Afficher le mot de passe"}>
