...
import type { Absence, AbsencesRespadonse } from '@/kpi/kpi'
...
export default function AbsenceList({ data, loading, onFixAbsences, onPageChange }: AbsenceListProps) {
  ...
  const handleFix = async (fixAll: boolean = false) => {
    setFixing(true)
    try {
      if (fixAll) {
        // Fix all absences (including unpaginated ones)
        await onFixAbsences([]) // Empty array signals fix all
      } else {
        // Fix only selected absences
